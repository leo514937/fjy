...
export function FileContentPanel({ selectedFile, fileContent, onRefresh }: FileContentPanelProps) {
  ...
  useEffect(() => {
    if (fileContent) {
      setViewContent(JSON.stringify(fileContent, null, 2));
    } else {
      setViewContent('');
    }
  }, [fileContent]);
  ...
  return (
    <Card className="border-border/40 bg-card/60 backdrop-blur-md shadow-2xl overflow-hidden flex flex-col h-[600px] rounded-3xl">
      <CardHeader className="bg-muted/10 border-b border-border/20 flex flex-row items-center justify-between px-6 py-4 group">
        <div>

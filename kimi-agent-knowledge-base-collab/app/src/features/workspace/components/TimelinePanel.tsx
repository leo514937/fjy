...
import type { XgTimelineCommit } from '@/lib/xgApi';
      ...
      {selectedFile ? commits.map((commit, index) => (
        <div key={commit.id} className="relative pl-8 pb-8 group">
          ...
          <div className="flex items-center gap-4 mt-1.5">
            <Badge variant="outline" className="text-[9px] px-1.5 font-black uppercase tracking-widest bg-muted/20 border-border/20">
              {commit.author}
            </Badge>
      ...
      <CardHeader className="pb-3 border-b border-border/20 bg-muted/10 px-6 py-4 flex flex-row items-center justify-between group">
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button variant="ghost" size="icon" className="h-6 w-6 rounded-md hover:bg-purple-500/10 text-muted-foreground hover:text-purple-500 transition-all">
          ...
          <DialogContent className="max-w-full w-screen h-screen flex flex-col rounded-none border-none bg-background p-0 gap-0 overflow-hidden shadow-none transition-colors duration-300">
            <div className="flex-1 min-h-0 relative">
              <ScrollArea className="h-full w-full">
                <TimelineList isFull {...commonProps} />
              </ScrollArea>

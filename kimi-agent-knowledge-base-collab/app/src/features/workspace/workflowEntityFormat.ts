...
function validateRelation(item: unknown, index: number): string {
...
export function validateWorkflowEntityFileData(data: unknown): { ok: boolean; error?: string } {
  ...
  if (!asObject(root.ontology_summary)) {

...
import { ensureKnowledgeIoWorkspace, resolveKnowledgeIoPaths } from "./ioPaths.mjs";
...
export function mergeIoHistoryContainers({ projectId, filename, containers }) {
  const mergedHistory = [];
...
async function migrateExportHistory({ containers, commitService, projectId, filename }) {
  const merged = mergeIoHistoryContainers({ projectId, filename, containers });
  const writes = [];
  for (let index = 0; index < merged.history.length; index += 1) {
    ...
    const result = await commitService.writeVersion({
      ...
      filename,
      data: payload,
      message: "System: append IO export snapshot",
...
export async function migrateKnowledgeIo(options = {}) {
  const paths = resolveKnowledgeIoPaths({
    workspaceRoot: options.workspaceRoot,
    env: options.env,
    defaultIoCodeRoot: options.defaultIoCodeRoot,
  });
  const ensured = ensureKnowledgeIoWorkspace(paths);
  const oldWikiRoot = path.resolve(options.oldWikiRoot || path.join(paths.ioCodeRoot, "wiki"));
  ...
  const targetProjectDir = path.join(paths.ioStorageRoot, options.projectId || "demo");
  ...
  if (await isNonEmptyDirectory(targetProjectDir)) {
    return {
      ...
      knowledgeIoRoot: paths.knowledgeIoRoot,
      targetProjectDir,
    };
  }

  const commitService = new OntoGitLocalCommitService({
    storageRoot: paths.ioStorageRoot,
  });
  ...
  const exportMigration = await migrateExportHistory({
    containers: [
      await readJsonIfExists(path.join(legacyProjectDir, "workflow_export.json")),
      await readJsonIfExists(path.join(oldStoreProjectDir, "workflow_export.json")),
    ].filter(Boolean),
    commitService,
    projectId: options.projectId || "demo",
  ...
  return {
    status: "success",
    knowledgeIoRoot: paths.knowledgeIoRoot,
    ioDocsRoot: ensured.ioDocsRoot,
    storeRoot: paths.ioStorageRoot,
    projectId: options.projectId || "demo",
    migratedSources,
